...
class LucaPPI4(PreTrainedModel):
    def __init__(self, config, args):
        super(LucaPPI4, self).__init__(config)
    ...
    def forward(
        ...
        hidden_states_a = self.decoder_a(
        ...
        hidden_states_b = self.decoder_b(
        ...
        if self.pooler_a is not None:
            hidden_states_a = self.pooler_a(hidden_states_a, mask=matrix_attention_masks_b)
        if self.pooler_b is not None:
            hidden_states_b = self.pooler_a(hidden_states_b, mask=matrix_attention_masks_b)
        if self.fusion_type == "add":
            hidden_states = torch.add(hidden_states_a, hidden_states_b)
        else:
            hidden_states = torch.cat([hidden_states_a, hidden_states_b], dim=-1)
