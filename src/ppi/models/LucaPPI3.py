...
class LucaPPI3(PreTrainedModel):
    def __init__(self, config, args):
        super(LucaPPI3, self).__init__(config)
        self.input_type = args.input_type
        self.num_labels = config.num_labels
        self.output_mode = args.output_mode
        self.task_level_type = args.task_level_type
        if config.hidden_size != args.embedding_input_size_a:
            self.linear_a = nn.Linear(args.embedding_input_size_a, config.hidden_size, bias=True)
        else:
            self.linear_a = None
        if config.hidden_size != args.embedding_input_size_b:
            self.linear_b = nn.Linear(args.embedding_input_size_b, config.hidden_size, bias=True)
        else:
            self.linear_b = None
